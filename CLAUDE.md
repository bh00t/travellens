...
- **Under active hardening — edit ONLY per a specific backlog item:**
  ...
  `ai/main.py` (B-004, B-005; B-062 one-line `filters`-merge), `ai/semantic_search.py` (B-006 dedup, B-004, polarity filter: `_detect_polarity` query-intent detector (B-062) drives a `sentiment_label` retrieval predicate (B-026 Stage 2 — swapped from B-062's star-rating bound; substantially resolves L-012, aspect-level residual L-017)),
...
├── run.py                           ← dev launcher (B-028 + B-030b + B-044 + B-045 + B-026): up the stack + 7 host procs (consumer, simulator, dashboard, gold_lifecycle_updater, review_embedder, quarantine_hourly_rollup, review_sentiment_scorer); --chaos; TAKES OVER on startup (B-045 newest-wins: _startup_cleanup() kills live run.py supervisor via port 47219 + all children by cmdline match; polls pg_locks until advisory locks 7400030/40/50/60/70 free; foreign-:5000 guard exits instead of blind-killing unrelated processes); then binds TCP 47219 as singleton (simultaneous double-start race → one exits "try again")
...
│   │                                  second instance exits 1. B-026 Stage 3: run.py's 7th managed proc,
│   │                                  launched bare (loop mode) — run.py owns its lifecycle like the embedder.
│   │                                  Run standalone: `python -m scripts.review_sentiment_scorer --once`.
